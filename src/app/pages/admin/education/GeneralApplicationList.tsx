...
export default function GeneralApplicationList() {
    ...
    const fetchApplications = async () => {
        setLoading(true);
        // Fetch applications with category 'general'
        // Joined with students to get name, phone, region
        // Joined with courses to get title, etc. (Optional if we hardcode or store in app)
        ...
        if (error) {
            console.error('Error fetching general apps:', error);
        } else if (data) {
            const mapped = data.map((item: any) => ({
                id: item.id,
                course_title: item.courses?.title || '일반과정',
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            <h1 className="text-2xl font-bold mb-6 text-gray-800">일반후계농 필수교육 신청자리스트</h1>
